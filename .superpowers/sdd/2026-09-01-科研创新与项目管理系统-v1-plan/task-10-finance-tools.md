...
本模块是“辅助工具”中的科研采购/经费支出简单登记，不扩展为财务核算系统。V1 核心只验收报销、发票、支付数据和附件能够登记、查询与安全留存；原有手工/自动匹配、确认、单据填写和模板生成仅作兼容保留，不作为扩展方向。不增加预算、总账、供应商、会计期间、审批链、财务接口、新角色或 AI 强依赖。交付包 `app/expense.db` 当前四张表均为 0 行，验收以程序、合成夹具和真实空库对账为准。
...
### 2026-09-04 范围纠偏与最终修复

- 用户再次确认财务只承担科研采购/经费支出的简单登记，不建设财务核算、预算控制、报销审批或复杂流转；本批没有增加上述业务能力，既有单据生成仅兼容保留。
- RED：真实页面没有分页入口、混合上传会吞掉手工整理提示；补充定向测试后首轮 `1 failed, 18 passed, 25 skipped`。
- 修复：全部数据库请求保持有界；报销项、发票、支付和单据列表提供最小上一页/下一页；发票和支付使用独立偏移；混合成功/失败上传同样通过 `textContent` 展示手工整理提示；页面金额明确标注“本页合计”；跨页深链直接按 ID 打开记录。
- GREEN：财务专项 `19 passed, 25 skipped`；隔离 PostgreSQL runner 六组 `1 + 11 + 2 + 1 + 44 + 54` 全部通过；全量 `368 passed, 73 skipped`；`compileall` 与 `git diff --check` 通过。
- 最终独立复核：Spec PASS；Quality PASS；Critical 0；Important 0。实现提交：`ee023bf`。
