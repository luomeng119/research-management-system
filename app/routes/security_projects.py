...
bp = Blueprint('security_projects', __name__, url_prefix='/security_projects')
...
def _get_project(project_id):
    ...
    if service is not None:
        try:
            return service.get_legacy(
                category='SECURITY_CONFIDENTIALITY', business_id=project_id
            )
        except ProjectServiceError:
            return None
    return SecurityProjectModel().get_by_id(project_id)
...
def index():
    ...
    if service is not None:
        result, status_groups = legacy_page(
            service, category='SECURITY_CONFIDENTIALITY', page=page, page_size=per_page,
        ...
        return render_template(template,
            ...
            visible_directories=['security_projects', 'equipment', 'standards'],
            project_type='security_projects', project_type_display='安全保密项目',
            page=page, total_pages=total_pages, total=total)

    all_projects = SecurityProjectModel().get_all()
...
def add():
    ...
    if request.method == 'POST':
        ...
        if service is not None:
            try:
                created = service.create_standalone('SECURITY_CONFIDENTIALITY', {
            ...
            project_id = SecurityProjectModel().add(project_id, name, leader, start_date, end_date_plan, end_date_actual, status)
    ...
    return render_template('projects/add.html', today=today, default_leader=current_user,
                           return_url=url_for('security_projects.index'))
...
def detail(project_id):
    ...
    return render_template('projects/detail.html', project=project, category='security', folder_tree=folder_tree, folder_types=RESEARCH_FOLDER_TYPES, project_equipment=project_equipment, available_equipment=all_equipment, available_equipment_total=resources['available_total'], project_groups=project_groups)
...
def upload_folder(project_id):
    return upload_project_folder(project_id, "SECURITY_CONFIDENTIALITY")
...
def preview_file(project_id, filepath):
    """文件预览接口"""
    controlled = controlled_project_download(project_id, "SECURITY_CONFIDENTIALITY", filepath, preview=True)
    if controlled is not None:
        return controlled
    if 'user' not in session:
        return jsonify({'success': False, 'message': '未登录'})
...
def delete_folder(project_id):
    return delete_project_folder(project_id, "SECURITY_CONFIDENTIALITY")

@bp.route('/delete_file/<project_id>', methods=['POST'])
def delete_file(project_id):
    controlled = delete_controlled_project_file(project_id, "SECURITY_CONFIDENTIALITY")
    if controlled is not None:
        return controlled
    if 'user' not in session:
        return jsonify({'success': False, 'message': '未登录'})
...
def archive_project(project_id):
    controlled = controlled_project_archive(project_id, "SECURITY_CONFIDENTIALITY")
    if controlled is not None:
        return controlled
    if 'user' not in session:
        return redirect(url_for('auth.login'))
...
def update_project_field(project_id):
    ...
    SecurityProjectModel().update(project_id, **{field_map[field]: value})
    return jsonify({'success': True, 'message': '更新成功'})
...
def delete(project_id):
    ...
    if service is not None:
        try:
            service.delete_legacy_standalone(category='SECURITY_CONFIDENTIALITY', business_id=project_id)
    ...
    if not project:
        return jsonify({'success': False, 'message': '项目不存在'})
    SecurityProjectModel().delete(project_id)
    return jsonify({'success': True, 'message': '删除成功'})
...
def project_documents(project_id):
    ...
    return render_template('projects/documents.html', 
                         project=project,
                         documents=documents,
                         document_blueprint='security_projects')
...
def upload_project_doc(project_id):
    ...
    return redirect(url_for('security_projects.project_documents', project_id=project_id))
