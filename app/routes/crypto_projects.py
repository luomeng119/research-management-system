...
def upload(project_id):
    return upload_project_file(project_id, "CRYPTO_APPLICATION")

@bp.route('/upload_folder/<project_id>', methods=['POST'])
def upload_folder(project_id):
    return upload_project_folder(project_id, "CRYPTO_APPLICATION")
...
def download_file(project_id, filepath):
    controlled = controlled_project_download(project_id, "CRYPTO_APPLICATION", filepath)
...
def rename_file(project_id):
    """文件名重命名"""
    controlled = rename_controlled_project_file(project_id, "CRYPTO_APPLICATION")
    if controlled is not None:
        return controlled
    if 'user' not in session:
        return jsonify({'success': False, 'message': '未登录'})
